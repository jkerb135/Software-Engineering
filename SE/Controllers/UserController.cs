...
namespace SE.Controllers
{
    ...
    public class UserController : ApiController
    {
        ...
        {
            return
                _db.aspnet_Users.ToList()
                    .Where(x => x.aspnet_Roles.Any(tl => tl.RoleName == "User"))
                    .Select(
                        tl =>
                            new User
                            {
                                ...
                                IsAnonymous = tl.IsAnonymous,
                                LastActivityDate = tl.LastActivityDate,
                                Password = tl.aspnet_Membership.Password
                            })
                    .AsEnumerable();
        }
        ...
        {
            return
                _db.aspnet_Users.Where(x => x.UserId == id)
                    .Select(
                        tl =>
                            new User
                            {
        ...
        public HttpResponseMessage PostMainStepCompleted([FromBody] CompletedMainStep mainstep)
        {
            ...
            CompletedMainStep complete =
                _db.CompletedMainSteps.FirstOrDefault(
                    u =>
                        u.AssignedUser == mainstep.AssignedUser && u.MainStepID == mainstep.MainStepID &&
                        u.MainStepName == mainstep.MainStepName && u.DateTimeComplete == mainstep.DateTimeComplete);
            if (complete != null)
        ...
        public HttpResponseMessage PostTaskCompleted([FromBody] CompletedTask task)
        {
            ...
            CompletedTask complete =
                _db.CompletedTasks.FirstOrDefault(
                    u =>
                        u.AssignedUser == task.AssignedUser && u.TaskID == task.TaskID && u.TaskName == task.TaskName &&
                        u.DateTimeCompleted == task.DateTimeCompleted);
            if (complete != null)
